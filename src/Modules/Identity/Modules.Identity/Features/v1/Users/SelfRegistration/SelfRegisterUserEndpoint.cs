...
public static class SelfRegisterUserEndpoint
{
    internal static RouteHandlerBuilder MapSelfRegisterUserEndpoint(this IEndpointRouteBuilder endpoints)
    {
        ...
        // Deliberately not idempotent: an anonymous route has no subject to bind the replay partition
        // to, so the key alone would hand a guessed key someone else's stored response (#84). A retry
        // is safe without it anyway — UserRegistrationService refuses a duplicate email/username with
        // 400, so no second user is ever created.
        .AllowAnonymous()
