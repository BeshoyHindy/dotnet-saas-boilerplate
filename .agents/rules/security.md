...
- **Response headers are stored by allow-list** — `Location`, `ETag`, `Content-Language`, plus the content type. `Set-Cookie` never reaches the cache, so no replay can hand one caller another's cookie. **Do not mark a token- or cookie-issuing endpoint idempotent**: for an anonymous caller the partition is the client-supplied key alone, so a guessed key would be handed that response body. None of the kit's four idempotent endpoints (register user, self-register, create tenant, request upload URL) issue tokens, and the anonymous auth routes that set the refresh cookie are deliberately not among them.
- **Weigh a short-lived capability the same way.** Request-upload-URL is idempotent and answers with a presigned PUT URL valid for minutes, from an entry that lives 24h: a retry under the same key past that expiry gets a dead link rather than a fresh one, and the presigned URL sits in the cache until the entry does. The partition is tenant + subject, so it stays a private file URL — a usability cost, not a leak. An endpoint minting a capability usable by somebody else must not be marked idempotent at all.
