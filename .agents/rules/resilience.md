...
```csharp
builder.Services.AddHttpClient("MyIntegration", ...)
...
- Only outbound integrations need this. Nothing in the template calls it today. Add it to any new typed/named `HttpClient` that calls a flaky external service.
- For internal ret/timeout of *background* work, prefer Hangfire's `[AutomaticRetry]` on the job (see `jobs.md`) — that's durable across restarts; the resilience handler only covers the in-flight HTTP call.
