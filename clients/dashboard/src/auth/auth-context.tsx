...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  // app shell that fires 401-ing requests.
  ...
  const login = useCallback(
    async (input: { email: string; password: string; tenant: string }) => {
      ...
      // cookie the same response set. Operators do not sign in here: platform
      // administration is the console's job (ADR-0008).
  ...
  const logout = useCallback(() => {
    ...
    //
    // It always names the signed-in user's own tenant and rides on their own session.
    ...
    // endSessionLocally() is the one place that clears the token store and the query
    // cache together (see .agents/rules/frontend/clients.md).
    endSessionLocally();
  }, []);
