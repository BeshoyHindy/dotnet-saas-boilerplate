import { api, unwrap, type Paged, type Schemas } from "@/lib/api-client";

// ────────────────────────────────────────────────────────────────────────
// Enum value mirrors. The *types* come from the generated contract; the const
// objects below exist only because a value is needed at call sites, and
// `satisfies` makes the compiler reject one the server no longer knows about.
// ────────────────────────────────────────────────────────────────────────
...
export type AuditSummaryDto = Schemas["AuditSummaryDto"];
export type AuditDetailDto = Schemas["AuditDetailDto"];
export type AuditSummaryAggregateDto = Schemas["AuditSummaryAggregateDto"];
...
/**
 * The audit endpoints bind `[AsParameters]` records, so their query keys are
 * PascalCase. Empty strings are dropped rather than sent as blank filters.
 */
function defined<T extends Record<string, unknown>>(query: T): T {
...
export async function listAudits(
  ...
  signal?: AbortSignal,
): Promise<Paged<AuditSummaryDto>> {
  return unwrap(
    await api.GET("/api/v1/audits", {
      signal,
      params: {
        query: defined({
          ...
          ExcludeEventType: query.excludeEventType,
          Severity: query.severity,
          Tags: query.tags === undefined ? undefined : String(query.tags),
          Source: query.source,
...
export async function getAuditById(id: string, signal?: AbortSignal): Promise<AuditDetailDto> {
  return unwrap(await api.GET("/api/v1/audits/{id}", { signal, params: { path: { id } } }));
}
...
export async function getAuditSummary(
  ...
  return unwrap(
    await api.GET("/api/v1/audits/summary", {
      signal,
      params: {
        query: defined({ FromUtc: query.fromUtc, ToUtc: query.toUtc, TenantId: query.tenantId }),
      },
    }),
  );
}
...
export async function getAuditsByCorrelation(
  ...
  return unwrap(
    await api.GET("/api/v1/audits/by-correlation/{correlationId}", {
      signal,
      params: {
        path: { correlationId },
        query: defined({ fromUtc: query.fromUtc, toUtc: query.toUtc }),
      },
    }),
  );
}
