...
// ────────────────────────────────────────────────────────────────────────
// Demo account picker — a two-pane "step into any role" dialog. Left rail =
// demo tenants, right pane = that tenant's seeded users. Tapping a user signs
// in instantly, with THAT account's tenant — which is the point: the person
// never types a tenant, or picks one out of a list of real customers.
//
// Driven by the static DEMO_ACCOUNT_GROUPS — no API call, since the login page
// is anonymous and an endpoint listing accounts would be enumeration.
// Visibility is the caller's responsibility (`env.demoMode`), and so is the
// password: it is runtime config, never in this repository. When none is
// configured the caller prefills instead of signing in, which is why this
// component renders `hint` rather than deciding anything itself.
// ────────────────────────────────────────────────────────────────────────

interface DemoAccountsDialogProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  /** Fired when a user is tapped — the caller decides what "pick" means. */
  onPick: (account: DemoAccount) => void;
  /** Footer line: what tapping a row will do, given the runtime config. */
  hint?: string;
}

export function DemoAccountsDialog({ open, onOpenChange, onPick, hint }: DemoAccountsDialogProps) {
